...
#
# Local-conditioning check: seed BOTH backends NEAR a NativeMinuit minimum and see
# whether they converge to the SAME point. If they agree locally, the cold-start
# NativeMinuit/iminuit split is a far-from-minimum / multi-basin conditioning effect,
...
function jm_from(x0, err0; S=1)
    m = NativeMinuit.Minuit(chi2_iam, collect(Float64,x0); error=collect(Float64,err0), strategy=S)
...
say("\nStep 1 — NativeMinuit minimum from the COLD seed:")
...
say(@sprintf("  NativeMinuit: fval=%.4f valid=%s", jf2, string(jv2)))
...
say(@sprintf("  NativeMinuit: fval=%.4f valid=%s", jf3, string(jv3)))
...
say(@sprintf("  NativeMinuit: fval=%.4f valid=%s", jf4, string(jv4)))
