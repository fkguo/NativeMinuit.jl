...
# which NativeMinuit's single MIGRAD descends to ≈330.75 (deeper than iminuit's
...
# NativeMinuit test dependencies.
#
# IMPORTANT — run it against the checkout you intend to test. The repo's
# `scripts` environment `dev`s NativeMinuit at a FIXED absolute path (the primary
# checkout). Running `--project=scripts` from a *git worktree* therefore
# silently loads the OTHER checkout's NativeMinuit, not this one. To test THIS
...
#   rm -rf ~/.julia/compiled/v*/NativeMinuit*/
#   julia -t 8 --project=scripts BenchmarkExamples/IAM_2Pformfactor/test_convergence_gap.jl
#
# (Any environment that provides the IAM deps AND resolves NativeMinuit to this
# checkout works.) The first @test below — `m.strategy == Strategy(1)` — fails
# loudly if the wrong NativeMinuit is loaded, so a stale-checkout run cannot
...
@testset "IAM cold-start convergence gap (docs/dev/IAM_CONVERGENCE_GAP.md)" begin
    ...
    fixL6!(m) = (NativeMinuit.fix!(m, 6); m)

    # The constructor default must be Strategy(1) (numerical FCN).
    m_probe = NativeMinuit.Minuit(chi2_iam, paras0; error = errs0)
    @test m_probe.strategy == NativeMinuit.Strategy(1)
    ...
    m = fixL6!(NativeMinuit.Minuit(chi2_iam, paras0; error = errs0))
    ...
    m1 = fixL6!(NativeMinuit.Minuit(chi2_iam, paras0; error = errs0))
    NativeMinuit.migrad!(m1; iterate = 1)
    ...
    m0 = fixL6!(NativeMinuit.Minuit(chi2_iam, paras0; error = errs0, strategy = 0))
    NativeMinuit.migrad!(m0)
