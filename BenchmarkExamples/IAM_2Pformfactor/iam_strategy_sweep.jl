...
#
# IAM strategy sweep — authoritative current-code fvals at S=0/1/2 for BOTH
# JuMinuit (native) and iminuit (via PyCall directly), single-shot (iterate=1)
# AND each library's default retry (iterate=5). Same cold seed `paras0`, same
# FCN (3 ππ datasets, pars[1:8]) as bench_full.jl / test_convergence_gap.jl.
#
# Run in a dedicated env (dev JuMinuit + PyCall + IAM deps); iminuit comes
# through the PyCall Python (same one IMinuit.jl uses).
#
...
let mdef = JuMinuit.Minuit(chi2_iam, paras0; error=errs0)
    say("  JuMinuit default-constructor strategy = $(mdef.strategy)  (must be Strategy(1))")
end
say("="^78)
