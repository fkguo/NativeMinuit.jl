...
#
# ─────────────────────────────────────────────────────────────────────────────
# NativeMinuitDataFramesExt — `Data(::DataFrame)` IMinuit.jl drop-in compat.
#
...
# NativeMinuit core API, breaking notebooks that pass DataFrames directly
...
# making it mandatory would inflate NativeMinuit's install footprint. Users
# who already have DataFrames loaded get the convenience method for free.
# ─────────────────────────────────────────────────────────────────────────────

module NativeMinuitDataFramesExt

using NativeMinuit
using DataFrames

"""
    NativeMinuit.Data(df::DataFrame)
...
```julia
using DataFrames, CSV, NativeMinuit
...
"""
NativeMinuit.Data(df::DataFrame) = NativeMinuit.Data(df[:, 1], df[:, 2], df[:, 3])
...
"""
function DataFrames.DataFrame(r::NativeMinuit.BootstrapResult)
...
"""
function DataFrames.DataFrame(r::NativeMinuit.JackknifeResult)
    return DataFrame(parameter = r.names,
                     estimate = r.estimate,
                     mean = r.mean,
                     bias = r.bias,
...
"""
    NativeMinuit.contour_df_samples(m::Minuit; kwargs...) -> DataFrame

`DataFrame` of the accepted Monte-Carlo parameter sets from
[`NativeMinuit.get_contours_samples`](@ref): one row per kept set, one column
per free parameter (named after the parameters), plus a `:delta_chisq`
column carrying each set's true Δχ². All
[`NativeMinuit.get_contours_samples`](@ref) keyword arguments are forwarded.

```julia
using DataFrames, NativeMinuit
df = contour_df_samples(m; nsamples = 30_000, cl = 1)
df.delta_chisq          # each kept set's χ²(x) − χ²_min (in χ² units)
```
"""
function NativeMinuit.contour_df_samples(m::NativeMinuit.Minuit; kwargs...)
    res = NativeMinuit.get_contours_samples(m; kwargs...)
...
end # module NativeMinuitDataFramesExt
