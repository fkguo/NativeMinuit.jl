...
@testset "linalg.jl" begin
    ...
    @testset "SYMMETRIC_UPLO convention" begin
        @test NativeMinuit.SYMMETRIC_UPLO === :U
    end
    ...
    @testset "sym_rank1_update!" begin
        ...
        NativeMinuit.sym_rank1_update!(S, α, x)
        ...
        y = [-1.0, 0.5, 2.0]
        β = -1.0
        NativeMinuit.sym_rank1_update!(S2, α, x)
        NativeMinuit.sym_rank1_update!(S2, β, y)
        ...
        z = rand(4)
        @test (@inferred NativeMinuit.sym_rank1_update!(S3, 0.3, z)) === S3
    end
    ...
    @testset "sym_mul!" begin
        ...
        # Default: y = S * x
        NativeMinuit.sym_mul!(y, S, x)
        @test y ≈ S * x

        # y = α S x
        NativeMinuit.sym_mul!(y, S, x, 2.0)
        @test y ≈ 2.0 * (S * x)

        # y = α S x + β y_init
        y_init = [1.0, -1.0, 2.0]
        y2 = copy(y_init)
        NativeMinuit.sym_mul!(y2, S, x, 2.0, 3.0)
        @test y2 ≈ 2.0 * (S * x) + 3.0 * y_init

        # Type stability
        y3 = zeros(3)
        @test (@inferred NativeMinuit.sym_mul!(y3, S, x)) === y3
    end
    ...
    @testset "Zero-allocation hot kernels" begin
        ...
        # Warmup (compile)
        NativeMinuit.sym_mul!(y, sym_S, x)
        NativeMinuit.sym_rank1_update!(sym_S, 0.1, x)

        # Measure
        @test (@allocated NativeMinuit.sym_mul!(y, sym_S, x)) == 0
        @test (@allocated NativeMinuit.sym_mul!(y, sym_S, x, 2.0, 3.0)) == 0
        @test (@allocated NativeMinuit.sym_rank1_update!(sym_S, 0.1, x)) == 0
    end
    ...
    @testset "sym_invert!" begin
        ...
        @test_throws LinearAlgebra.SingularException NativeMinuit.sym_invert!(S_sing)

        # throw_on_fail=false returns S without throwing on singular
        S_sing2 = Symmetric(zeros(2, 2), :U)
        @test NativeMinuit.sym_invert!(S_sing2; throw_on_fail = false) === S_sing2
    end
    ...
    @testset "sym_eigvals" begin
        ...
        evs = NativeMinuit.sym_eigvals(S)
        @test length(evs) == 2
        @test issorted(evs)
        # λ = (7 ± √5) / 2
        ...
        @test NativeMinuit.sym_eigvals(S_I) ≈ ones(3) atol = 1e-14
    end
    ...
    @testset "sum_sym matches C++ Σ|M[i,j]| (mndasum semantics)" begin
        ...
        S_L = Symmetric(M_L, :L)
        # |1| + |-2| + |3| + |4| + |-5| + |6| = 21
        @test NativeMinuit.sum_sym(S_L) == 21.0
    end
end
